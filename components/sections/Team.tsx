  ...
  return (
    <li className="group border-t border-bone/15 last:border-b">
      <button
        ...
        className="block w-full cursor-pointer py-[clamp(2rem,5vw,4rem)] text-left"
      >
        {/* Top mono row: index · role, and the expand hint. */}
        ...
        {/* Big name + the 3D mark. */}
        <div className="flex items-center justify-between gap-5">
          <motion.h3
            style={{ y: nameY }}
            className="outline-name min-w-0 font-display font-medium uppercase leading-[0.9] tracking-[-0.04em] text-[clamp(1.75rem,8vw,5.5rem)]"
          >
            {member.name}
          </motion.h3>
          <Mark3D shape={member.shape} className="shrink-0" />
        </div>
      </button>

      {/* Focus blurb (optional click-to-expand, one open at a time). */}
...
export default function Team() {
  ...
  // Desktop: the two names parallax at different rates as the section scrolls (depth).
  ...
  return (
    <section
      id="team"
      ref={ref}
      className="px-6 py-[clamp(6rem,12vw,11rem)] sm:px-10 lg:px-16"
    >
